...
const Navbar = () => (
  <header className="screen-max-width flex items-center justify-between p-5 sm:px-10">
    ...
    <nav className="text-gray flex flex-1 justify-center gap-10 text-sm max-sm:hidden">
      {navLists.map((nav, i) => (
        <a key={i} className="group relative cursor-pointer transition-colors hover:text-white">
          {nav}
          <span className="absolute bottom-0.5 left-1/2 h-0.5 w-0 bg-white transition-[width_transform] group-hover:-bottom-px group-hover:left-[10%] group-hover:w-[80%] motion-safe:duration-300" />
        </a>
      ))}
    </nav>
